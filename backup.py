...
#a = topic_yr_map
#x = distinct_yrs
def viz_tot(a, x, a_new, x_new): # a = yr_topic_map
    ...
    a_vals = list(a_new.values())
    y = list(zip(*a_vals))

    xnew = np.linspace(min(x_new),max(x_new),200) #300 represents number of points to make between T.min and T.max

    for i in range(0,10):
        power_smooth = spline(x_new,y[i],xnew)
        plt.plot(xnew,power_smooth, label="Topic"+str(i))
    plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
...
# Main Fn Call
topic_yr_map_new = inter1d()
print(topic_yr_map_new)

# print(topic_yr_map.keys())
viz_tot(topic_yr_map,x,topic_yr_map_new,pred_yrs)