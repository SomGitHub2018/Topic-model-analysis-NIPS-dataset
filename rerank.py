...
output_unsupervised = "output_unsupervised"

label_list =[]
with open(output_candidates,'r') as k:
    for line in k:
        labels = line.split()
...
if test_chunk_size < int(num_unsup_labels):
    print "\n"
    print "Error"
...
def get_best_label(label_list,num):
    ...
    return [i[0] for i in list_sorted[:int(num_unsup_labels)]]
...
print "\n"
g = open(output_unsupervised,'w')
for i,item in enumerate(unsup_output):
    print "top " +str(num_unsup_labels)+ " labels for topic " +str(i) +" are:"
    g.write("top " +str(num_unsup_labels)+ " labels for topic " +str(i) +" are:" +"\n")
    for elem in item:
        print elem
        g.write(elem +"\n")
    print "\n"
